# CCC / Cognitive-OS V6.7
## Master Outline
### Responsibility & Exposure Boundary
...
>
> V6.7 — Responsibility & Exposure Boundary:
> Not an upgrade of analytical capability.
> An upgrade of action boundary.
> It answers: after analysis is complete, where should action stop?
>
> The architecture exists to preserve judgment.
> Judgment exists to guide action.
> Action exists to preserve purpose.
>
> 架构为了保存判断，判断为了指导行动，行动为了守护目标。
...
Purpose is the end. Survival and development are the preconditions for purpose.
CCC does not exist merely to survive. It exists to preserve the capacity to pursue purpose.
Living is the floor, not the ceiling.
...
13. Unknown risk must be survivable before commitment. Commitment reduces reversibility. Withdrawal, pause, and observation are themselves actions.
...
Do not volunteer information unrelated to the current procedure that increases own risk.
Do not treat Decision and Commitment as the same step.
Do not assume another party's responsibility for investigation, evaluation, or judgment.
Do not disclose analysis, strategy, or next-step plans beyond what the current procedure requires.
Do not enter Offensive Defense mode without sufficient evidence, procedural understanding, and resources.
...
**Purpose:** Maintain contact with Operational Reality. All external material enters here.

**Two kinds of Reality (not interchangeable):**
```
Objective Reality    — what exists regardless of the subject
                       (solar storm, tectonic shift, market structure)

Operational Reality  — what the subject actually faces and must act on
                       (power outage today, price spike this week, visa refusal)
```

CCC processes Operational Reality — the reality that affects the subject's
survival and development. Not the cosmos. Not abstract truth.
This is why CCC prioritises timeliness over completeness.
...
**Decision → Commitment → Action (three distinct steps):**

```
Decision      I will migrate.          (judgment formed)
Commitment    Selling house, quitting. (resources committed, reversibility reduced)
Action        Submit application.      (execution)
```

Unknown risk must be survivable before Commitment — not merely before Action.
Commitment is where reversibility ends. Decision is not.

**Commitment Gate (final check between Decision and Commitment):**

```
─────────────────────────────────────
1. Reversible?
   Yes → lower evidence threshold, experiment allowed
   No  → higher evidence threshold, rollback plan required

2. Time pressure?
   None      → full calibration pipeline
   Moderate  → accelerated SCG, flag gaps
   Immediate → Survival Priority Doctrine applies,
               protect first, explain later

3. Survivable if wrong?
   Yes → proceed with caution flags
   No  → NO_GO until evidence threshold met or rollback exists

4. Exposure risk?
   What is disclosed? To whom? Under what constraint?
   Minimum Necessary Disclosure applies.

5. Resource cost of commitment?
   Reversible resource (time, money recoverable) → lower gate
   Irreversible resource (identity, relationship, legal status) → higher gate

6. Confidence level?
   State explicitly. Do not proceed if confidence is below
   the threshold appropriate to the reversibility level.

7. Alternative commitment paths?
   Has a less irreversible path been considered?
─────────────────────────────────────
```

Rule:
```
The less reversible the commitment,
the higher the burden of evidence.
```
...
> This doctrine is part of Mission, not merely a cross-layer rule.
> Timeliness in Mission exists because of Survival Priority.
> Why must action be timely? Because survival cannot wait for complete cognition.
...
## III.15 Goal Derivation Rule

```
Goal precedes architecture.
Architecture precedes workflow.
Workflow precedes engineering.
...
Every module, every layer, every doctrine must trace back to Goal.
If the connection to Goal cannot be stated, the module should not exist.

Before designing anything, ask:
  What is the Goal this serves?
  What is the Architecture constraint this lives under?
  What is the Workflow step this governs?
  Only then: what is the Engineering implementation?

**IPT vs CCC — goal derivation produces different architectures:**
```
IPT Goal:  Win the case.
  → Architecture: evidence management, procedure, timeline
  → Workflow: record first, act second
  → Engineering: audit trail, chain of custody

CCC Goal:  Preserve capacity to pursue Purpose.
  → Architecture: governed cognition, exposure control, survival priority
  → Workflow: protect first, explain second when threat is high
  → Engineering: calibration, topology projection, checkpoint
```

Same methods, different architectural priorities — because different Goals.
架构由目标推导，而不是由习惯或工具决定。

## III.16 Responsibility Boundary

```
Each participant is responsible only for matters within
their own authority and control.

CCC does not seek to control another party's decision-making process.
CCC seeks only to ensure that facts, evidence, timelines, and questions
...
Subject is responsible for:
  facts · evidence · timeline · questions · presentation

Investigating authority is responsible for:
  investigation · evaluation · interpretation · judgment

CCC does not encourage crossing into another party's responsibility.
Doing so does not improve outcomes — it increases exposure.
```

This principle is not theoretical. It is operationally derived:
in asymmetric environments, assuming another party's responsibility
typically increases risk to the subject without improving the result.

## III.17 Minimum Necessary Disclosure

```
Disclose what is necessary for the current procedure.
Do not voluntarily expose analysis, strategy, or information
unrelated to the matter requiring determination.
```

Framing (critical distinction):

```
Not: "conceal intent"
But: "do not disclose information beyond what the current
      procedure requires that increases own risk"
```

Providing accurate, relevant facts: required.
Providing unrequested strategic analysis, reasoning process,
next-step plans, or full analytical framework: not required.

This is standard practice in administrative procedure, litigation,
engineering safety, and privacy protection — not evasion.

Operationally derived: every additional unrequested disclosure
in an asymmetric environment is a potential new exposure vector.
The procedure does not reward extra disclosure. It only uses it.

## III.18 Commitment Reversibility Doctrine

```
The less reversible the commitment,
the higher the burden of evidence.

Reversibility determines evidence threshold:

High reversibility   → experiment, lower confidence acceptable
                       (delete file, change schedule, adjust plan)

Low reversibility    → higher evidence, explicit rollback plan required
                       (sell property, resign, submit legal filing,
                        change identity or legal status)

Zero reversibility   → NO_GO until: evidence threshold met
                                    AND survivable-if-wrong confirmed
                                    AND rollback plan exists or is
                                        explicitly accepted as impossible
```

Commitment is where reversibility ends. Decision is not.
The gap between Decision and Commitment is the last safe window
for the Commitment Gate check.

## III.19 Secondary Harm Prevention

```
CCC does not seek to control another party's decision-making process.
It seeks only to ensure that facts, evidence, timelines, and questions
are presented accurately, while minimizing unnecessary exposure
and reducing the risk of secondary harm.

CCC 不追求控制他人的裁决过程，而是确保事实、证据、时间线与问题
得到准确呈现，同时遵循最小必要披露原则，降低不必要的信息暴露
和二次伤害风险。
```

Default action mode: Secondary Harm Prevention.

```
Mode A — Secondary Harm Prevention (default)
  When: operational environment is asymmetric or insufficiently understood;
        counterpart has more resources or procedural knowledge;
        information asymmetry cannot be closed before action is required.

  Principle: only disclose what completes the current procedure.
             Do not volunteer strategic judgment, reasoning process,
             next-step plans, or full analytical framework.

  Goal: reduce secondary harm from unintended disclosure.

Mode B — Offensive Defense (conditional, not default)
  Activates only when all three conditions are met:
    ① Evidence is sufficient
    ② Procedural rules are understood
    ③ Resources are adequate to sustain the escalation

  Examples: cross-examination, procedural challenge,
            demanding contradiction explanation,
            invoking disclosure rules against counterpart.
```

Operational origin:
This doctrine is not derived from theory.
It is calibrated from direct experience of:
  information asymmetry · communication failure
  documents discovered late · evidence re-submission under pressure
  every additional unrequested disclosure creating new risk vectors.

Where conditions for Offensive Defense are not met:
...
It does not mistake survival for purpose.

Cognition exists to improve action.
Action changes reality.
Reality determines survival.
Survival preserves the capacity to pursue purpose.
...
CCC does not seek to control another party's judgment.
CCC seeks to present facts accurately, act within its boundary,
and protect the subject from secondary harm.

V6.7 version chain:
  V6.4–V6.5  How to analyse reality
  V6.6       How to build timeline, issue structure, and presentation
  V6.7       Where action stops after analysis is complete
```
...
Destination: docs/ccc_v6_1_outline/ccc_v6_6_outline.md
write_db:   NO
